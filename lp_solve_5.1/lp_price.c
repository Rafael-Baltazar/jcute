...
doLoop:
  ...
  if(lp->spx_trace) {
    if(current.varno > 0)
      report(lp, DETAILED, "col_prim: Column %d reduced cost = " RESULTVALUEMASK "\n",
...
Retry:
  ...
  for(; pass > 0; pass--) {
    ...
    for(; i*iz <= iy*iz; i += iz) {
      ...
      if(fabs(f) < epspivot) {
        if(lp->spx_trace)
          report(lp, FULL, "rowprim: Pivot " RESULTVALUEMASK " rejected, too small (limit " RESULTVALUEMASK ")\n",
                           f, epspivot);
      }
...
{
  int      k, i, iy, iz, ii;
  REAL     rh, lo, up, epsvalue, epsdelta;
  pricerec current, candidate;
  MYBOOL   collectMP = FALSE;
  int      *multivars = NULL;
...
doLoop:
  ...
  for(; k*iz <= iy*iz; k += iz) {
   ...
   /* Analyze relevant constraints ...
    ...
    if((rh < lo) || (rh > up+epsdelta) || ((fabs(rh) < epsvalue) && (eliminate == TRUE))) {
      if(rh > up)
        rh = up-rh;
  ...
  if(lp->spx_trace) {
    if(current.varno > 0) {
      report(lp, DETAILED, "row_dual: rhs[%d] = " RESULTVALUEMASK "\n",
                           current.varno, lp->rhs[current.varno]);
      if(lp->upbo[lp->var_basic[current.varno]] < lp->infinite)
        report(lp, DETAILED, "\t\tupper bound of basic variable:    %18g\n",
                             lp->upbo[lp->var_basic[current.varno]]);
    }
    else
...
STATIC REAL normalizeEdge(lprec *lp, int item, REAL edge, MYBOOL isdual)
{
  ...
  if(is_piv_strategy(lp, PRICE_RANDOMIZE))
    edge *= (1.0-PRICER_RANDFACT) + PRICER_RANDFACT*(myrand() % RANDSCALE) / (REAL) RANDSCALE;
#endif
  return( edge );
