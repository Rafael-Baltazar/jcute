 ...
 /* Must initialize due to computation of scaling statistic - KE */
...
#endif
  for(; i <= lp->rows; i++)
    scalechange[i] = minmax_to_scale(lp, row_min[i], row_max[i], mat_rowlength(lp->matA, i));
  ...
  for(j = 1; j <= lp->columns; j++) {
    if(is_int(lp,j) && !is_integerscaling(lp)) { /* do not scale integer columns */
      ...
      scalechange[lp->rows + j] = minmax_to_scale(lp, col_min, col_max, mat_collength(lp->matA, j));
    }
  }
