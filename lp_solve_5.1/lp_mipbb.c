  ...
  if(status != OPTIMAL) {
    ...
    if(lp->spx_status != OPTIMAL)
      ...
      if(lp->lag_status != RUNNING) {
        report(lp, NORMAL, "\nRelaxed solution  " RESULTVALUEMASK " found as B&B basis.\n",
                           lp->solution[0]);
        report(lp, NORMAL, " \n");
      }
  ...
  *varcus = 0;
  ...
  if((lp->bb_limitlevel != 1) && (MIP_count(lp) > 0)) {
    ...
    if((lp->int_count > 0) && (*varno == 0)) {
      *varno = find_int_bbvar(lp, &countnint, BB->lowbo, BB->upbo);
      if(*varno > 0)
        *vartype = BB_INT;
    }
    ...
    if(*varno == 0) {
      ...
      /* Current solution is better */
        ...
        if(lp->bb_trace ||
          ...
          report(lp, IMPORTANT,
                 "%s solution " RESULTVALUEMASK " found at iteration %8d, %7d nodes explored (gap %.1f%%)\n",
  ...
  *varcus = countnint;
  if(MIP_count(lp) > 0) {
    if((lp->solutioncount == 1) && (lp->solutionlimit == 1) &&
      ...
      if(lp->spx_trace)
        report(lp, DETAILED, "B&B level %d OPT %s value " RESULTVALUEMASK "\n",
